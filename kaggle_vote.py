...
glob_files = sys.argv[1]    # 要进行融合的文件
loc_outfile = sys.argv[2]   # 输出结果文件
weights_strategy = "uniform"
if len(sys.argv) == 4:
  weights_strategy = sys.argv[3]  # 获取权重参数

def kaggle_bag(glob_files, loc_outfile, method="average", weights="uniform"):
  pattern = re.compile(r"(.)*_[w|W](\d*)_[.]*")
  if method == "average":
    scores = defaultdict(list)
  with open(loc_outfile,"w",encoding='UTF-8') as outfile:
    ...
    for i, glob_file in enumerate( glob(glob_files) ):
      ...
      lines = open(glob_file,encoding='UTF-8').readlines()
