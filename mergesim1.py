...
df3 = pd.read_table('./train1_skep_result.txt',header=None)
df1.columns = ['label']
df2.columns = ['label']
df3.columns = ['label']
...
df3 = df3[['id','label']]
# 保存结果文件
df1.to_csv('./train1_robert_result.csv', index=False)
df2.to_csv('./train1_nezha_result.csv', index=False)
df3.to_csv('./train1_skep_result.csv', index=False)

# 筛选多份提交结果文件中预测相同的部分
result_path   = 'train1_robert_result.csv'
...
forest_path = 'train1_nezha_result.csv'
...
forest_path1 = 'train1_skep_result.csv'
...
t3 = t3[['text_a','label']]
t3.to_csv('newtest.csv',index=False,sep=',')
print('Done!')